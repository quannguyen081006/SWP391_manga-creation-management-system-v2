...
<%--
  Full notification list (up to 100 rows). Mark-all-read uses a plain POST form;
  delete and read/unread toggle use fetch() via shared header.js (loaded by header.jsp).
  Relative timestamps (noti-time) are filled by header.js timeAgo — not JSTL.
--%>
<!DOCTYPE html>
<html>
<head>
...
<div class="page-actions">
    <%-- Mark-all-read: web POST route only; no JS required. --%>
...
<div class="notification-list">
    <c:choose>
        <%-- Empty state keeps the page useful when the user has no notification rows. --%>
        ...
        <c:otherwise>
            <c:forEach items="${notifications}" var="n">
                <%-- Read/unread classes are used by CSS and the existing notification JS. --%>
                <div class="notification-row noti-item ${n.read ? 'is-read read' : 'is-unread unread'}" data-noti-id="${n.id}" data-is-read="${n.read}">
                    ...
                    <%-- Delete/toggle: API + header.js; no web form routes for these actions. --%>
